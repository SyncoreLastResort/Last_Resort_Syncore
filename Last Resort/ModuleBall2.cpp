...
bool ModuleBall2::CleanUp()
{
	...
	if (ball2_collider != nullptr)

		ball2_collider->to_delete = true;
...
void ModuleBall2::MoveAround()
{
	...
	{
		if (position.y >= App->player2->position.y - 35 || position.x != App->player2->position.x + 8)
		{
			if (position.x >= App->player2->position.x + 8 && position.y <= App->player2->position.y - 1)
			{
				position.x -= speed;
			}
			else if (position.x >= App->player2->position.x + 8 && position.y > App->player2->position.y - 1)
			{
				position.x += speed;
			}
			else if (position.x < App->player2->position.x + 8 && position.y <= App->player2->position.y - 1)
	...
	{
		if (position.x <= App->player2->position.x + 42)
		{
			if (position.x >= App->player2->position.x + 8 && position.y <= App->player2->position.y - 1)
			{
				position.y += speed;
	...
	{
		if (position.x >= App->player2->position.x - 26)
		{
			if (position.x >= App->player2->position.x + 8 && position.y <= App->player2->position.y - 1)
			{
				position.y -= speed;
	...
	//Limits
	if (position.x <= App->player2->position.x - 26)
		position.x = App->player2->position.x - 26;

	if (position.x >= App->player2->position.x + 42)
		position.x = App->player2->position.x + 42;

	if (position.y >= App->player2->position.y + 35)
		position.y = App->player2->position.y + 35;
...
{
	if (App->input->keyboard[SDL_SCANCODE_LEFT] == KEY_STATE::KEY_REPEAT && (App->player2->position.x - App->player2->speed >= 0))
	{
		position.x -= App->player2->speed;
	}
	if (App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_REPEAT && (App->player2->position.x + App->player2->speed <= SCREEN_WIDTH - 32))
	{
		position.x += App->player2->speed;
	}
	if (App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_REPEAT && (App->player2->position.y - App->player2->speed > 0))
	{
		position.y -= App->player2->speed;
	}
	if (App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_REPEAT && (App->player2->position.y + App->player2->speed <= SCREEN_HEIGHT - 14))
	{
		position.y += App->player2->speed;
	}
	...
	else if (App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_REPEAT&&App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_REPEAT)
		current_animation = &blueball_225;

	else if (App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_REPEAT&&App->input->keyboard[SDL_SCANCODE_LEFT] == KEY_STATE::KEY_IDLE && App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_IDLE)
		current_animation = &blueball_270;
	...
	else if (App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_REPEAT&&App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_REPEAT)
		current_animation = &blueball_135;

	else if (App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_REPEAT&&App->input->keyboard[SDL_SCANCODE_LEFT] == KEY_STATE::KEY_IDLE && App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_IDLE)
		current_animation = &blueball_90;

	else if (App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_REPEAT &&App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_IDLE &&App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_IDLE)
		current_animation = &blueball_180;

	else if (App->input->keyboard[SDL_SCANCODE_LEFT] == KEY_STATE::KEY_REPEAT &&App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_IDLE &&App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_IDLE)
		current_animation = &blueball_0;
...
BALL_POSITION ModuleBall2::getPosition()
{
	...
	if (position.x < App->player2->position.x + 15 && position.x >= App->player2->position.x - 10 && position.y < App->player2->position.y - 1 && position.y > App->player2->position.y - 25)
		ret = LEFT_TOP;

	else if (position.x < App->player2->position.x + 10 && position.x >= App->player2->position.x - 10 && position.y>App->player2->position.y - 1 && position.y < App->player2->position.y + 25)
		ret = LEFT_BOTTOM;

	else if (position.x <= App->player2->position.x - 10)
		ret = LEFT_SIDE;

	else if (position.x >= App->player2->position.x + 15 && position.y < App->player2->position.y - 1 && position.y > App->player2->position.y - 25 && position.x < App->player2->position.x + 41)
		ret = RIGHT_TOP;

	else if (position.x >= App->player2->position.x + 10 && position.y > App->player2->position.y - 1 && position.y < App->player2->position.y + 25 && position.x < App->player2->position.x + 41)
		ret = RIGHT_BOTTOM;

	else if (position.x >= App->player2->position.x + 41)
		ret = RIGHT_SIDE;

	else if (position.y <= App->player2->position.y - 25)
		ret = TOP;
